(function(){
  ...
  const visible=()=>{const w=innerWidth; if(w>1450)return 6; if(w>1100)return 5; if(w>640)return 3; return 1;};
  ...
  function update(){
    ...
    if(dots){
      const pages=Math.min(8,Math.ceil(reviews.length/visible()));
      const active=Math.min(pages-1,Math.floor(index/visible()));
      dots.innerHTML=Array.from({length:pages},(_,i)=>`<button aria-label="Opinie ${i+1}" class="${i===active?'active':''}"></button>`).join('');
    }
  }
  ...
  function start(){stop();timer=setInterval(()=>go(1),3900);} function stop(){if(timer)clearInterval(timer);}
  const card=track.closest('.trust-card'); card?.addEventListener('mouseenter',stop); card?.addEventListener('mouseleave',start);
