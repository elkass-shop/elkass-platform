...
const brands=['samsung','bosch','lg','beko','amica','philips','sony','lenovo','whirlpool','electrolux','siemens','tefal','jbl','sharp','tcl','hisense','gorenje','xiaomi','aeg','panasonic','remington','blaupunkt','kernau','sencor','manta','philco','miele','liebherr','haier','candy','delonghi','asus','acer'];
...
function renderProducts(list){
 const active=(Array.isArray(list)?list:[]).filter(p=>p.is_active!==false); if(!active.length) active.push(...builtInProducts); const hit=active.find(p=>String(p.id).includes('samsung'))||active[0]; const side=active.filter(p=>p.id!==hit?.id).slice(0,4); const grid=active.slice(0,8);
